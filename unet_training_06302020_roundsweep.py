...
train_path ='/media/hd1/unet_model_training_data_cm/unet-master/data_'+channel+'/unet_data/round_'+rnd+'_train_little-delta_'+little_delta+'_total-time_'+total_time
...
steps_per_epoch = 2000 #int(len(glob.glob(train_path+os.sep+image_folder+os.sep+'*.png'))/batch_size)
...
model_checkpoint = ModelCheckpoint('/media/hd1/unet_model_training_data_cm/unet-master/data_'+channel+'/round_'+rnd+'_06302020_unet_little-delta_'+little_delta+'_total-time_'+total_time+'.hdf5', monitor='loss',verbose=1, save_best_only=True)
...
try:
	...
	#Test the model
    test_path ='/media/hd1/unet_model_training_data_cm/unet-master/data_'+channel+'/unet_data/round_'+rnd+'_test_little-delta_'+little_delta+'_total-time_'+total_time
    ...
    steps = 50 #int(len(glob.glob(test_path+os.sep+image_folder+os.sep+'*.png')))
    ...
    fil = '/media/hd1/unet_model_training_data_cm/unet-master/data_'+channel+'/round_'+rnd+'_06302020_unet_little-delta_'+little_delta+'_total-time_'+total_time+'.hdf5'
...
t2 = time.time()
T = {'little-delta':int(little_delta),'total_time':int(total_time),'channel':channel,'round':int(rnd),'time':(t2-t1)/60.,'status':status,
...
if not os.path.isfile('/media/hd1/unet_model_training_data_cm/unet-master/MasterResults_rounds.csv'):
    df.to_csv('/media/hd1/unet_model_training_data_cm/unet-master/MasterResults_rounds.csv')
else:
    with open('/media/hd1/unet_model_training_data_cm/unet-master/MasterResults_rounds.csv', 'a') as f:
