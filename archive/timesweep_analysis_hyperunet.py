...
from model import unet_hyperspectral
...
MAX_NUMBER_OF_FRAMES = int(sys.argv[1])
base_dir1 = sys.argv[2]
base_dir2 = sys.argv[3]
...
model1 = unet_hyperspectral()
model2 = unet_hyperspectral()
#Load the file weights
ch='gfp'
weights1_file = '/home/meyerct6/Data/ml_antibiotics/unet-hyperspectral/data_'+ch+'/unet_little-delta_nested_total-time_'+str(MAX_NUMBER_OF_FRAMES)+'.hdf5'
ch = 'bf'
weights2_file = '/home/meyerct6/Data/ml_antibiotics/unet-hyperspectral/data_'+ch+'/unet_little-delta_nested_total-time_'+str(MAX_NUMBER_OF_FRAMES)+'.hdf5'
...
for f1,f2,cnt in zip(chunked_iterable(fils1,size=batch_size),chunked_iterable(fils2,size=batch_size),chunked_iterable(fil_cnter,size=batch_size)):
    print('Currently working on images: ' + str(cnt[0]) + '-' + str(cnt[-1]) + ' of ' + str(len(fils2)) + ' images')

    df_file = pd.DataFrame({'file_gfp':f1,'file_bf':f2})
    #Use a generator which uses flow_from_dataframe (df_file_sub)
    predict_generator1 = predictGenerator(len(cnt),df_file,x_col='file_gfp',image_color_mode = "grayscale",target_size = (256*256,10))
    predict_generator2 = predictGenerator(len(cnt),df_file,x_col='file_bf',image_color_mode = "grayscale",target_size = (256*256,10))
    ...
    df.loc[cnt,'per_back_gfp']      = np.sum(np.sum(r1==0,2),1)/(256**2*10)
    df.loc[cnt,'per_back_bf']       = np.sum(np.sum(r2==0,2),1)/(256**2*10)
    df.loc[cnt,'per_back_combo']    = np.sum(np.sum(r3==0,2),1)/(256**2*10)
...
df.to_csv('05282020_all_results_little-delta_nested_total-time_'+str(MAX_NUMBER_OF_FRAMES)+'.csv')
